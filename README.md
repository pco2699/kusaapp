...
- 🔀 **Sort & filter** — a 28px strip folded above the date row (collapsed by default, so the board stays the board). Sort by your **own order** (⇅ — drag habits by the ☰ grip that appears while the strip is open, or move them with the arrow keys; saved server-side), by **strength** (💪) or by **the order you added them** (🕒); show **only what's due today** (🎯) or **everything** (📋). It's all icons, with an ⓘ legend, and an active filter keeps showing its icon on the collapsed strip so a short board is never a mystery
